(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.formxtra-cf7-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.formxtra-cf7-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.formxtra-cf7-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.formxtra-cf7-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: FORMXTRACF7CORE.admin_ajax,
        type: "POST",
        data: {
            action: "formxtra_cf7_recommended_activate_plugin",
            file: file,
            nonce: FORMXTRACF7CORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "formxtra_cf7_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: FORMXTRACF7CORE.recommended_nonce
        },
