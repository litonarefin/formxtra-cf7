	...
	public function dashboard_widget_render()
	{
		...
		if (!empty($maxitems)) {
			...
			foreach ($rss_items as $item) {
				...
				// $item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'formxtra-cf7' );
		...
		<style>
			...
			.formxtra-cf7-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.formxtra-cf7-rss-widget .formxtra-cf7-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.formxtra-cf7-rss-widget .formxtra-cf7-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.formxtra-cf7-rss-widget .formxtra-cf7-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.formxtra-cf7-rss-widget .formxtra-cf7-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.formxtra-cf7-rss-widget .formxtra-cf7-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	?>
		...
		<script>
			...
			function updateCounter(seconds) {
				const $counter = $container.find(".formxtra-cf7-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".formxtra-cf7-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".formxtra-cf7-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".formxtra-cf7-popup-countdown").hide(0);
				}
