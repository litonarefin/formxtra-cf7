...
if ( ! class_exists( 'Loader' ) ) {
	...
	final class Loader extends Client {
		...
		public function setup_refresh_event() {
			...
			register_deactivation_hook( FORMXTRACF7_FILE, array( get_class( $this ), 'clear_refresh_event' ) );
		...
		public function config_page() {
			...
			<div id="<?php echo esc_attr( self::get_prefixed( 'formxtra-cf7-popup' ) ); ?>" class="formxtra-cf7-license-popup formxtra-cf7-license-settings-page
								<?php
								if ( $is_license_active ) {
									echo 'formxtra-cf7-license-status--active'; }
								?>
			">
				<div class="formxtra-cf7-license-popup--wrapper formxtra-cf7-license-popup--display">
					<div class="formxtra-cf7-license-popup--container">
						<div class="formxtra-cf7-license-popup--container-inner">

							<div class="formxtra-cf7-license-popup--notice"></div>

							<?php if ( $is_license_active ) { ?>

								<h3><?php esc_html_e( 'License Key Deactivation', 'formxtra-cf7' ); ?></h3>

								<p>
									<?php esc_html_e( 'Deactivates an License Key Key so it can be used on another blog.', 'formxtra-cf7' ); ?>
								</p>

								<table class="form-table">
									<tbody>
										<tr>
											<th scope="row"><?php esc_html_e( 'License Key', 'formxtra-cf7' ); ?></th>
										...
										<tr class="formxtra-cf7-license-status">
											<th scope="row"><?php esc_html_e( 'License Status', 'formxtra-cf7' ); ?></th>
											<td><?php echo esc_html( $license_status ); ?></td>
										</tr>
									</tbody>
								</table>

								<div class="formxtra-cf7-license-actions">
									<button class="formxtra-cf7-license-deactivate button button-primary wc-btn-active"><?php esc_html_e( 'Deactivate License', 'formxtra-cf7' ); ?></button>
							...
							<?php } else { ?>
								...
								<h3><?php esc_html_e( 'License Key Activation', 'formxtra-cf7' ); ?></h3>
								...
								<table class="form-table">
									<tbody>
										...
										<?php if ( $this->no_product_id ) { ?>
											<tr>
												<th scope="row"><?php esc_html_e( 'Product ID', 'formxtra-cf7' ); ?></th>
								...
								<div class="formxtra-cf7-license-actions">
									<button class="formxtra-cf7-license-activate button button-primary wc-btn-active"><?php esc_html_e( 'Activate License', 'formxtra-cf7' ); ?></button>
									<input type="hidden" name="nonce" value="<?php echo esc_js( wp_create_nonce( self::get_prefixed( 'admin-notice-nonce' ) ) ); ?>">
							...
							<div class="formxtra-cf7-license--loader-wrapper">
								<div class="formxtra-cf7-license--loader"></div>
							</div>
						</div>
					</div>
				</div>
		...
		public function inactive_notice() {
			...
			if ( apply_filters( 'wc_am_client_inactive_notice_override', true ) ) {
				...
				<div class="notice notice-error">
					<p><?php printf( __( 'The <strong>%1$s</strong> API Key has not been activated, so the %2$s is inactive! %3$sClick here%4$s to activate <strong>%5$s</strong>.', 'formxtra-cf7' ), esc_attr( $this->software_title ), esc_attr( $this->plugin_or_theme ), '<a href="' . esc_url( admin_url( 'admin.php?page=' . $menu_slug ) ) . '">', '</a>', esc_attr( $this->software_title ) ); ?></p>
				</div>
		...
		public function activate_license() {
			...
			if ( ! wp_verify_nonce( $nonce, self::get_prefixed( 'admin-notice-nonce' ) ) ) {
				wp_send_json_error(
					array(
						'message' => __( 'Oops! Security nonce is invalid.', 'formxtra-cf7' ),
					)
				);
			}
				...
				if ( ! empty( $activation_result ) ) {
					...
					if ( true === $activate_results['success'] && true === $activate_results['activated'] ) {
						...
						wp_send_json_success(
							array(
								'message'        => sprintf( __( '%s activated. ', 'formxtra-cf7' ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ),
								'activated_text' => __( 'Activated', 'formxtra-cf7' ),
							)
						);
					}
					...
					if ( false === $activate_results && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						...
						wp_send_json_error(
							array(
								'message' => esc_html__( 'Connection failed to the License Key API server. Try again later. There may be a problem on your server preventing outgoing requests, or the store is blocking your request to activate the plugin/theme.', 'formxtra-cf7' ),
							)
						);
					}
					...
					wp_send_json_error(
						array(
							'message' => esc_html__( 'The License Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', 'formxtra-cf7' ),
						)
					);
				}
			}
			...
			wp_send_json_error(
				array(
					'message' => __( 'License or Product ID is invalid.', 'formxtra-cf7' ),
				)
			);
		}
		...
		public function deactivate_license() {
			...
			if ( empty( $api_key ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'The License Key is missing from the deactivation request.', 'formxtra-cf7' ),
					)
				);
			}
			...
			if ( 'Activated' === $activation_status ) {
				...
				if ( ! empty( $deactivation_result ) ) {
					...
					if ( true === $activate_results['success'] && true === $activate_results['deactivated'] ) {
						...
						wp_send_json_success(
							array(
								'message' => esc_html__( 'License Key deactivated. ', 'formxtra-cf7' ) . esc_attr( "{$activate_results['activations_remaining']}." ),
							)
						);
					}
			...
			wp_send_json_success(
				array(
					...
					'message' => esc_html__( 'The License Key is already deactivated.', 'formxtra-cf7' ),
				)
			);
		}
		...
		public function license_popup_link( $links ) {
			$popup_target = self::get_prefixed( 'formxtra-cf7-popup' );

			if ( 'Activated' === $this->get_activation_status() ) {
				$links['license_key'] = '<a href="#" class="formxtra-cf7-lpt-btn inactive" data-wc-popup-target="' . esc_attr( $popup_target ) . '" aria-label="' . esc_attr__( 'Settings', 'formxtra-cf7' ) . '">' . esc_html__( 'Deactivate License', 'formxtra-cf7' ) . '</a>';
			} else {
				$links['license_key'] = '<a href="#" class="formxtra-cf7-lpt-btn active" data-wc-popup-target="' . esc_attr( $popup_target ) . '" aria-label="' . esc_attr__( 'Settings', 'formxtra-cf7' ) . '">' . esc_html__( 'Activate License', 'formxtra-cf7' ) . '</a>';
			}
		...
		public function admin_scripts( $hook = '' ) {
			wp_enqueue_style( self::get_prefixed( 'formxtra-cf7-license' ), plugin_dir_url( __FILE__ ) . '/assets/license.css', null, time() );
			wp_enqueue_script( self::get_prefixed( 'formxtra-cf7-license' ), plugin_dir_url( __FILE__ ) . '/assets/license.js', array( 'jquery' ), time(), true );
			...
			wp_localize_script( self::get_prefixed( 'formxtra-cf7-license' ), 'Formxtra_CF7_License_Manager_Vars', $localize_vars );
		}
		...
		public function view_license_popup() {
			...
			<div id="<?php echo esc_attr( self::get_prefixed( 'formxtra-cf7-popup' ) ); ?>" class="formxtra-cf7-license-popup
								<?php
								if ( $is_license_active ) {
									echo 'formxtra-cf7-license-status--active';
								}
					...
					<div class="formxtra-cf7-license-popup--container">
						<div class="formxtra-cf7-license-popup--container-inner">
							...
							<?php if ( $is_license_active ) { ?>
								...
								<h3><?php esc_html_e( 'License Deactivation', 'formxtra-cf7' ); ?></h3>

								<p>
									<?php esc_html_e( 'Deactivates an License Key so it can be used on another blog.', 'formxtra-cf7' ); ?>
								</p>
